...
function parseStatus(status: CollatedTrain, api: string): APIStatus {
    if (api === "times-api") {
        ...
        const parsedLocation = parseTimesAPILocation(timesAPIData.lastEvent.location);
        return {
...
export async function start(api: string) {
    ...
    const trains = Object.entries((await proxy.getTrains() as FullTrainsResponse).trains)
        .filter(([_, entry]) => {
    ...
    }, { trainProps: [`status.${apiCode}`] }).close;
}
